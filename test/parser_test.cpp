...
TEST(Parser, ParseOnceV1) {
  ...
  for (auto [span, expect] : v) {
    const auto [name, temperature] = ParseOnce_V1(span);
    EXPECT_EQ(name, expect.name);
    EXPECT_EQ(temperature, expect.temperature);
    EXPECT_TRUE(span.empty());
  }
...
TEST(Parser, ParseOnce) {
  ...
  for (auto [span, expect] : v) {
    const auto [name, temperature] = ParseOnce<FindFirstZeroByte_SWAR, ParseNumber_SWAR_V2>(span);
    EXPECT_EQ(name, expect.name);
    EXPECT_EQ(temperature, expect.temperature);
    EXPECT_TRUE(span.empty());
  }
